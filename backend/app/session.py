...
def slot_hints(sid, slot, known=None):
    ...
    if sid == "SC07" and slot == "sum_insured":
        ...
        pt = (known or {}).get("property_type")
        if pt in h["price_per_year_kzt"]: h["price_per_year_kzt"] = h["price_per_year_kzt"][pt]
    ...
    def _apply(self, f: Frame, st: Step):
        ...
        if st.kind == "ask":
            ...
            if f.asked[st.slot] >= 2:
                st.facts = {**(st.facts or {}), "repeat": True, "client_last_answer": self.history[-1]["text"] if self.history else None, **slot_hints(f.sid, st.slot, f.slots)}
