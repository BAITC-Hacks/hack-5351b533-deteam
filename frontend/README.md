...
Mock хранит звонки в памяти вкладки: звонки из симулятора видны в журнале, пока вкладка не перезагружена.
...
| `/supervisor` | Журнал звонков | супервизор | Сводка (решено роботом, точность, задержки, переспросы, оператор, какие сценарии робот путает) и **все** звонки: путь по сценариям, языки, время ответа, итог, флаги проблем. Фильтры — только для удобства, по умолчанию «Все». Обновляется сам |
| `/supervisor/calls/:id` | Карточка звонка | супервизор | Лента тем, полный диалог с итогом роутинга под каждым ответом, трассировка выбранной реплики, разметка «верно / ошибка → правильный сценарий» |
...
| GET | `/api/sessions` | — | `CallSummary[]` — **все** звонки для журнала супервизора, новые сверху |
| GET | `/api/sessions/{id}` | — | `CallDetails` = { summary, turns } — карточка звонка |
...
src/
  ...
  hooks/
    ...
    usePolling.ts     автообновление журнала и карточки звонка (живой режим)
  ...
  components/
    ...
    supervisor/       сводка, путь по сценариям, разметка реплик
  pages/              Simulator / Supervisor (журнал) / Call (карточка звонка) / Scenarios
```
